...
//Compute a sub-block for MMult1
void MMini(long m, long n, long k,
  long ar, long br, long cc, 
  double *a, double *b, double *c){
/*sa : starting block column for a
  sb : starting block row for b
  sc : starting row to place result*/
  for (long j = cc; j < cc+BLOCK_SIZE; j++) {
    for (long p = br; p < br+BLOCK_SIZE; p++) {
      for (long i = ar; i < ar+BLOCK_SIZE; i++) {
        double A_ip = a[i+p*m];
        double B_pj = b[p+j*k];
        double C_ij = c[i+j*m];
        C_ij = C_ij + A_ip * B_pj;
...
//Re-arrange loops to get max performance
void MMult1Arr(long m, long n, long k, double *a, double *b, double *c) {
  for (long p = 0; p < k; p++) {
    for (long i = 0; i < m; i++) {
      for (long j = 0; j < n; j++) {
        double A_ip = a[i+p*m];
...
void MMult1(long m, long n, long k, double *a, double *b, double *c) {
  // //Solve a block multiplication with each call. Do this for all blocks
  // for(int i = 0; i < m; i=i+BLOCK_SIZE){ //Each row block
  //   for(int j = 0; j < n; j=j+BLOCK_SIZE){ //Each column block
  //     for(int pt = 0; pt < k; pt=pt+BLOCK_SIZE){ // intermediate
  //        MMini(m, n, k, i, pt, j, a, b, c);
  //      }
  //   }
  // }
  ...
  for(int pt = 0; pt < k; pt=pt+BLOCK_SIZE){ //intermediate
    ...
    #pragma omp for
    for(int i = 0; i < m; i=i+BLOCK_SIZE){ //Each row block
      for(int j = 0; j < n; j=j+BLOCK_SIZE){ //Column
         MMini(m, n, k, i, pt, j, a, b, c);
       }
      }
    }
...
int main(int argc, char** argv) {
  ...
  for (long p = PFIRST; p < PLAST; p += PINC) {
    ...
    double time = t.toc();
    double flops = n*m*k*10*NREPEATS/1e9/time; // TODO: calculate from m, n, k, NREPEATS, time
    double bandwidth = n*m*k*4*NREPEATS/1e9/time; // TODO: calculate from m, n, k, NREPEATS, time
